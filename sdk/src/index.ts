 ...
 * `Wane.base()` so integrators never hand-paste an address. Both Base mainnet
 * (chain id 8453) and Base Sepolia are live; addresses are verified on BaseScan.
 */
...
export class Wane {
  ...
  static baseSepolia(cfg: Partial<WaneConfig> & { agent?: Address } = {}): Wane {
    ...
    return new Wane({ registry: d.registry, policy: d.policy, delegate: d.delegate, token: d.token, chain: d.chain, ...cfg });
  }
  ...
  static base(cfg: Partial<WaneConfig> & { agent?: Address } = {}): Wane {
    ...
    if (!d) {
      throw new Error("No Base mainnet deployment configured. Use Wane.baseSepolia() or pass addresses explicitly.");
    }
    return new Wane({ registry: d.registry, policy: d.policy, delegate: d.delegate, token: d.token, chain: d.chain, ...cfg });
  }
  ...
  async checkPolicy(
    ...
    const [allowed, reason] = (await this.pc.readContract({
      ...
      args: useCall
        ? [this.agent, tgt, selector, amount]
        : [this.agent, tgt, amount],
    })) as [boolean, number];
      ...
      if (looksLikeAttack && this.token && opts.wallet) {
        try {
          await this.report(opts.wallet, {
            ...
            evidence: keccak256(encodePacked(["string"], [String((err as Error)?.message ?? "attack")])),
          });
        } catch {
  ...
  async wouldAllow(call: WaneCall, account?: Address): Promise<PolicyVerdict> {
    ...
    const who = account ?? this.agent;
    if (!who) throw new Error("wouldAllow needs the protected wallet address (account or config.agent)");
    const [allowed, reason] = (await this.pc.readContract({
      address: who,
      abi: waneDelegateAbi,
  ...
  async sendBatch(wallet: WaneWallet, calls: WaneCall[]): Promise<Hex> {
    ...
    if (!(await this.isProtected(account.address))) {
      throw new Error(
        "wallet is not protected: call wane.enable(wallet) once before sendBatch().",
      );
    }
   ...
   */
  wrap(wallet: WaneWallet): { sendTransaction: (call: WaneCall) => Promise<Hex>; sendBatch: (calls: WaneCall[]) => Promise<Hex> } {
    return {
