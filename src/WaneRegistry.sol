...
contract WaneRegistry is ReentrancyGuard {
    ...
    event ParamsSet(uint96 mintStake, uint96 challengeStake, uint64 maturity, uint16 publisherBps, uint96 checkFee);
    ...
    constructor(address waneToken, address treasury_) {
    ...
    function checkBytecode(bytes32 codehash) external view returns (bool active, uint64 id) {
    ...
    function _enforceable(WaneTypes.Antibody storage a) internal view returns (bool) {
    ...
    function mintAntibody(WaneTypes.ThreatKind kind, bytes32 subject, bytes32 evidence)
        external
        nonReentrant
        notPaused
        returns (uint64 id)
    ...
    function corroborate(uint64 id) external notPaused {
    ...
    function challenge(uint64 id) external nonReentrant notPaused {
        WaneTypes.Antibody storage a = antibodies[id];
        if (a.id == 0) revert Unknown();
        if (a.status != WaneTypes.Status.Active) revert NotActive();
    ...
    function resolve(uint64 id, bool falsePositive) external onlyGovernor nonReentrant {
        WaneTypes.Antibody storage a = antibodies[id];
    ...
    function reclaimStake(uint64 id) external nonReentrant {
        WaneTypes.Antibody storage a = antibodies[id];
    ...
    function payCheck(uint64 id) external nonReentrant notPaused {
    ...
    function setEnforcement(uint64 window_, uint32 corrobs_) external onlyGovernor {
    ...
    function _key(WaneTypes.ThreatKind kind, bytes32 subject) internal pure returns (bytes32) {
