...
contract WanePolicy {
    ...
    function setEnabled(address agent, bool on) external onlyAgentOwner(agent) {
        Policy storage p = policies[agent];
        p.enabled = on;
    ...
    function setPaused(address agent, bool p_) external {
        Policy storage p = policies[agent];
    ...
    function setGlobalPaused(bool p_) external {
        if (msg.sender != guardian) revert NotGuardian();
        globalPaused = p_;
        emit GlobalPausedSet(p_);
    }

    function setGlobalDenied(address target, bool value) external {
        if (msg.sender != guardian) revert NotGuardian();
    ...
    function setAllow(address agent, address target, bool value) external onlyAgentOwner(agent) {
        allowlist[agent][target] = value;
        emit ListSet(agent, target, true, value);
    }

    function setBlock(address agent, address target, bool value) external onlyAgentOwner(agent) {
        blocklist[agent][target] = value;
        emit ListSet(agent, target, false, value);
    }

    function setSelectorScoped(address agent, bool scoped) external onlyAgentOwner(agent) {
        policies[agent].selectorScoped = scoped;
        emit SelectorSet(agent, 0x00000000, false, scoped);
    }

    function setSelector(address agent, bytes4 selector, bool value) external onlyAgentOwner(agent) {
        allowedSelector[agent][selector] = value;
        emit SelectorSet(agent, selector, value, policies[agent].selectorScoped);
    }

    function setTokenScoped(address agent, bool scoped) external onlyAgentOwner(agent) {
        policies[agent].tokenScoped = scoped;
        emit TokenSet(agent, address(0), false, scoped);
    }

    function setToken(address agent, address token, bool value) external onlyAgentOwner(agent) {
    ...
    function _evaluate(
        ...
        if (haveSelector && p.blockKinds & K_CALL != 0 && selector != 0x00000000) {
            (bool fc, uint64 idc) = registry.check(WaneTypes.ThreatKind.CallPattern, bytes32(selector));
    ...
    function recordSpend(address agent, uint128 amount) external onlyAgentOwner(agent) {
        Policy storage p = policies[agent];
    ...
    function usedToday(address agent) external view returns (uint128) {
        return _usedToday(policies[agent]);
    }

    function _usedToday(Policy storage p) internal view returns (uint128) {
        uint64 today = uint64(block.timestamp / 1 days);
        return p.dayStart == today ? p.spentToday : 0;
    }

    function _meetsSensitivity(uint64 id, uint32 minCorrobs) internal view returns (bool) {
